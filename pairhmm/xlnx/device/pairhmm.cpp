...
#include "device/pairhmm.h"

#define CAT_NX(A, B) A ## B
#define CAT(A, B) CAT_NX(A, B)

cellData CAT(computePipeline, SLRID)(custom_float factor, custom_float p_MM, custom_float p_MX, custom_float p_MY, cellData up, cellData left, cellData diag, float& resultMX, short c, ap_uint<8> r, custom_float Y_firstRow){
#pragma HLS inline
    cellData ret;
    if(c <= 1){
        diag.M = 0.0;
...
float CAT(computePE, SLRID)(cellData buff_diag[MAX_READ_LEN], cellData buff_upleft[MAX_READ_LEN], cellData buff_cur[MAX_READ_LEN], ap_uint<8> r, short c, float Y_firstRow, float factor, float p_MM, float p_MX, float p_MY, cellData& upleft, cellData& diag){
#pragma HLS inline
    cellData data_diag;
    cellData data_up;
    cellData data_left;
    ...
    data_cur = CAT(computePipeline, SLRID)((custom_float)factor, (custom_float)p_MM, (custom_float)p_MX, (custom_float)p_MY, data_up, data_left, data_diag, resultMX, c, r, (custom_float)Y_firstRow);

    buff_cur[r] = data_cur;
    return resultMX;
}

void CAT(computePEWrap, SLRID)(ap_uint<4> _rs, ap_uint<4> _hap, float p_MM, float ph2pr_div3_val, float ph2pr_sub1_val, float& cur_result, ap_uint<3> dmod3, cellData buff[3][MAX_READ_LEN], ap_uint<8> r, short c, float Y_firstRow, float p_MX, float p_MY, cellData& upleft, cellData& diag){
#pragma HLS inline
    bool equal = (_rs == _hap || _rs == 4 || _hap == 4);
    float factor = equal ? ph2pr_sub1_val : ph2pr_div3_val;
    float resultMX;
    ap_uint<2> dmod3_n, dmod3_nn;
    if(dmod3 == 0){ dmod3_n = 1; dmod3_nn = 2;}
    else if(dmod3 == 1){ dmod3_n = 2; dmod3_nn = 0;}
    else { dmod3_n = 0; dmod3_nn = 1;}
    cellData local_upleft, local_diag;
    local_upleft = upleft;
    local_diag = diag;
        resultMX = CAT(computePE, SLRID)(buff[dmod3], buff[dmod3_n], buff[dmod3_nn],\
                r, c, Y_firstRow, factor, p_MM, p_MX, p_MY, local_upleft, local_diag);
    upleft = local_upleft;
    diag = local_diag;
...
void CAT(computeEngine, SLRID)(short numHap, hls::stream<readType>& readDataCh, hls::stream<hapLenType>& hapLenCh, hls::stream<hapTypeBundle>& haplotypeBasesCh, hls::stream<accDataBundle>& results, hls::stream<short>& max_colsCh, hls::stream<short>& numReadsCh, hls::stream<longlong>& totalIterNumCh){
#pragma HLS inline off
    const float ph2pr[128] = PH2PR_INIT;
    const float ph2pr_div3[128] = PH2PR_DIV3_INIT;
#pragma HLS RESOURCE variable=ph2pr_div3 core=ROM_2P_BRAM
    const float ph2pr_sub1[128] = PH2PR_SUB1_INIT;
#pragma HLS RESOURCE variable=ph2pr_sub1 core=ROM_2P_BRAM

    ap_uint<16> hapLen[HAP_BLOCK_SIZE][MAX_HAPDATA_NUM / HAP_BLOCK_SIZE];
#pragma HLS array_partition variable=hapLen complete dim=1
#pragma HLS RESOURCE variable=hapLen core=RAM_1P_LUTRAM
    
    ap_uint<32> Yrow1[HAP_BLOCK_SIZE][MAX_HAPDATA_NUM / HAP_BLOCK_SIZE];
#pragma HLS array_partition variable=Yrow1 complete dim=1
#pragma HLS RESOURCE variable=Yrow1 core=RAM_1P_LUTRAM

    ap_uint<64> haplotypeBases[MAX_HAPDATA_NUM / HAP_BLOCK_SIZE * MAX_HAP_LEN / 4];
#pragma HLS RESOURCE variable=haplotypeBases core=XPM_MEMORY uram
    char kmod = 0;
    int addr = 0;
    int k = 0;
consume_hap_len:
...
#pragma HLS unroll
            ...
            if(oneOrTwo && big_rows < rows[1])
                big_rows = rows[1];
            ap_uint<8> new_rows = big_rows;
            if(new_rows < DEP_DIST)
                new_rows = DEP_DIST;
            if(oneOrTwo){
                read_valid[0] = true; read_valid[1] = true;
            }
            else{
                read_valid[0] = true; read_valid[1] = false;
            }
...
#pragma HLS unroll
                ...
                accData tmp_data;
                CAT(computePEWrap, SLRID)(_rs[iidx], _hap[idx], p_MM[iidx], ph2pr_div3_val[iidx], ph2pr_sub1_val[iidx], tmp_data.val,\
                        dmod3, buff[index], r, c, Y_firstRow[idx], p_MX[iidx], p_MY[iidx], \
...
void CAT(accEngine, SLRID)(hls::stream<accDataBundle>& results, hls::stream<accData>& output, hls::stream<int>& numResults){
#pragma HLS inline off
...
void CAT(accBusEngine, SLRID)(int expected, hls::stream<accData> results[PU_NUM], float* output){
#pragma HLS inline off
    int index = 0;
    accData tmp;
#pragma HLS data_pack variable=tmp
...
template<unsigned int PU_NUM>
void CAT(dispatcher, SLRID)(ap_uint<512>* input_data, short numRead, short numHap, hls::stream<readType> readData[PU_NUM], hls::stream<hapLenType> hapLen[PU_NUM], hls::stream<hapTypeBundle> haplotypeBases[PU_NUM], hls::stream<short> max_cols[PU_NUM], hls::stream<short> numReadsCh[PU_NUM], hls::stream<int> numResultsCh[PU_NUM], hls::stream<longlong> totalIterNumCh[PU_NUM]){
#pragma HLS inline off
    int readLenPtr = 0;
    int readDataPtr = 0;
    int hapLenPtr = 0;
...
produce_hap_len:
    for(int k = 0; k < numHap; k++){
        ...
        if(hapLen_tmp + 1 > maxCols)
            maxCols = hapLen_tmp + 1;
...
#pragma HLS resource variable=hapBurstBuf core=xpm_memory uram
...
#pragma HLS resource variable=readBurstBuf core=xpm_memory uram
    readType readFeedBuf[PU_NUM][MAX_READ_LEN * READ_BLOCK_SIZE];
#pragma HLS resource variable=readFeedBuf core=xpm_memory uram
...
#pragma HLS resource variable=readInfoBurstBuf core=xpm_memory uram
...
#pragma HLS resource variable=readInfoFeedBuf core=xpm_memory uram
...
template<unsigned int PU_NUM>
void CAT(computePairhmm, SLRID)(ap_uint<512>* input_data, short numRead, short numHap, int iterNum, float* output_data){
#pragma HLS inline off
#pragma HLS dataflow
    hls::stream<accData> output_fifo[PU_NUM];
#pragma HLS stream variable=output_fifo depth=8
#pragma HLS data_pack variable=output_fifo
...
#pragma HLS data_pack variable=hapLen
...
#pragma HLS stream variable=totalIterNumCh depth=2
    CAT(dispatcher, SLRID)<PU_NUM>(input_data, numRead, numHap, readData, hapLen, haplotypeBases, max_cols, numReadsCh, numResultsCh, totalIterNumCh);
    for(int idx = 0; idx < PU_NUM; idx++){
#pragma HLS unroll
        CAT(computeEngine, SLRID)(numHap, readData[idx], hapLen[idx], haplotypeBases[idx], acc_fifo[idx], max_cols[idx], numReadsCh[idx], totalIterNumCh[idx]);
        CAT(accEngine, SLRID)(acc_fifo[idx], output_fifo[idx], numResultsCh[idx]);
    }
    CAT(accBusEngine, SLRID)<PU_NUM>(iterNum, output_fifo, output_data);
}

template<unsigned int PU_NUM>
void CAT(pmm_core, SLRID)(ap_uint<512>* input_data, int numRead, int numHap, float* output_data){
#pragma HLS inline off
    int numIters = numRead * numHap;
    CAT(computePairhmm, SLRID)<PU_NUM>(input_data, numRead, numHap, numIters, output_data);
}

#ifndef HLS_SIM
extern "C" { 
#endif
void CAT(pmm_core_top, SLRID)(ap_uint<512>* input_data, int numRead, int numHap, float* output_data){
#pragma HLS INTERFACE m_axi port=input_data offset=slave bundle=input_data depth=50330
#pragma HLS INTERFACE m_axi port=output_data offset=slave bundle=output_data depth=262144
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
    CAT(pmm_core, SLRID)<SLR_PE_NUM / (READ_BLOCK_SIZE * HAP_BLOCK_SIZE)>(input_data, numRead, numHap, output_data);
}
#ifndef HLS_SIM
}
#endif
