...
int FalconPairHMM::computePairhmmBaseline(pairhmmInput* input, pairhmmOutput* output, bool use_double){
    ...
    for(int i = 0; (size_t)i < input->reads.size(); i++){
        ...
        for(int j = 0; (size_t)j < input->haps.size(); j++){
            ...
            if(result_float < MIN_ACCEPTED){
                ...
                //result_final = log10(result_double) - log10(ldexp(1.0, 1020.0));
            }
            else{
                result_final = (double)(log10f(result_float) - g_ctxf.LOG10_INITIAL_CONSTANT);
                //result_final = (double)(log10f(result_float) - log10f(ldexpf(1.f, 120.f)));
            }
...
int FalconPairHMM::computePairhmmAVX(pairhmmInput* input, pairhmmOutput* output, bool use_double){
    testcase testCase;
    output->likelihoodData.clear();
    for(int i = 0; (size_t)i < input->reads.size(); i++){
        testCase.rslen = input->reads[i].bases.size();
...
int FalconPairHMM::get_max_rsdata_num(){
...
void FalconPairHMM::convert_read_input(readDataPack* cur_host_read, Read* input, int idx){
...
void FalconPairHMM::distributeReads(pairhmmInput* input, int read_base_index, int hap_base_index, int& numRead0, int& numRead1, int& numRead2, int& totalNumRead, int& totalNumHap, short maxCols, bool& violate){
...
void FalconPairHMM::computePairhmmAVXSegment(pairhmmInput* input, int read_base_index, int hap_base_index, int cur_numRead, int cur_numHap, vector<float>& output){
    testcase testCase;
...
void FalconPairHMM::sortReads(pairhmmInput* input, int read_base_index, int cur_numRead, int cur_numHap, short maxCols, int slr_pu_num[3], bool& violate){
    ...
    for(int k = 0; k < DIE_NUM; k++){
        ...
        for(int j = 0; (size_t)j < sortedReadInfo[k].size(); j+=slr_pu_num[k]){
            ...
            if((size_t)(j + upper_bound) >= sortedReadInfo[k].size())
    ...
    for(int slr_id = 0; slr_id < DIE_NUM; slr_id++){
        for(int i = 0; (size_t)i < sortedReadInfo[slr_id].size(); i++){
            for(int j = 0; j < sortedReadInfo[slr_id][i].readLen[0]; j++){
                /*  if(input->reads[sortedReadInfo[slr_id][i].readID[0] + read_base_index + read_start_index].bases[j] == 'N'){
                    violate = true;
                    return;
                }*/
            ...
            if(sortedReadInfo[slr_id][i].oneOrTwo){
                for(int j = 0; j < sortedReadInfo[slr_id][i].readLen[1]; j++){
                    /* if(input->reads[sortedReadInfo[slr_id][i].readID[1] + read_base_index + read_start_index].bases[j] == 'N'){
                        violate = true;
                        return;
                    }*/
    ...
    for(int slr_id = 0; slr_id < 3; slr_id++){
        ...
        for(int k = 0; k < slr_pu_num[slr_id]; k++){
            ...
            //printf("slr %d, PU %d has %d iterations and %d reads in total\n", slr_id, k, host_input[slr_id]->dataPack.iterNum[k], host_input[slr_id]->dataPack.numReadPU[k]); 
            total_slr_itercount += host_input[slr_id]->dataPack.iterNum[k];
        }
        //printf("slr %d total has %lld iters \n", slr_id, total_slr_itercount);
    }
...
void FalconPairHMM::update_host_inputs_new(pairhmmInput* input, int read_base_index, int hap_base_index, int cur_numRead, int cur_numHap, bool& violate){
...
void FalconPairHMM::update_host_inputs(pairhmmInput* input, int read_base_index, int hap_base_index, int cur_numRead, int cur_numHap, bool& violate){
    short maxCols = 0;
    for(int i = 0; i < cur_numHap; i++){
        if(input->haps[i + hap_base_index].bases.size() > MAX_HAP_LEN){
            violate = true;
    ...
    for(int i = 0; i < cur_numRead; i++){
        ...
        if(i < host_input[0]->numRead){
            ...
            for(int j = 0; j < cur_read_len; j++){
                convert_read_input(&(host_input[1]->dataPack.readData[offset][j]), &(input->reads[i + read_base_index]), j);
            }
        }
        else if(DIE_NUM > 2){
            ...
            for(int j = 0; j < cur_read_len; j++){
                convert_read_input(&(host_input[2]->dataPack.readData[offset][j]), &(input->reads[i + read_base_index]), j);
            }
        }
    }
...
#ifdef HLS_SIM
void convert_bus_input(char* src, ap_uint<512>* bus, int length){
    for(int i = 0; i < length; i++){
        ap_uint<512> tmp;
        for(int j = 0; j < 64; j++){
            tmp(8 * j + 7, 8 * j) = src[i * 64 + j];
        }
        bus[i] = tmp;
    }
}
...
int FalconPairHMM::computePairhmmFalcon(pairhmmInput* input, pairhmmOutput* output, bool& usedFPGA){
    struct timespec time1, time2, time_diff;
    short maxCols = 0;
 ...
 //   int max_rsdata_num = get_max_rsdata_num();
   ...
   // printf("numReadSeg = %d, numHapSeg = %d\n", numReadSeg, numHapSeg);
    for(int i = 0; i < numReadSeg; i++){
        ...
        for(int j = 0; j < numHapSeg; j++){
            ...
            if(violate){
                computePairhmmAVXSegment(input, read_base_index, hap_base_index, cur_numRead, cur_numHap, host_raw_output);
            }
            else{
                usedFPGA = true;
                computePairhmmFPGA((char*)&(host_input[0]->dataPack), host_input[0]->numRead, host_input[0]->numHap, host_output[0], \
                ...
                for(int k = 0; k < cur_numHap * cur_numRead; k++){
                    ...
                    if(k < host_input[0]->numRead * cur_numHap)
                        ...
                        cur_float_result = host_output[2][k - host_input[0]->numRead * cur_numHap - host_input[1]->numRead * cur_numHap];
    ...
    for(int k = 0; k < numRead * numHap; k++){
        if(host_raw_output[k] < MIN_ACCEPTED){
            ...
            double result_double = compute_fp_avxd(&testCase);
            //printf("%d th data = %f overflowed, recomputing, recomputed result = %f\n", k, host_raw_output[k], result_double);
...
//#define JACOBIAN_LOG_TABLE_SIZE 80001
...
float setMatchToMatchProbMerlin(char insQual,char delQual,float *jacobianLogTable){
    ...
    float LN10 = log(10.0);
    float output_data;
    //double temp0 = POW(((double )10),(approximateLog10SumLog10Merlin(((double )(- 0.1)) * ((double )((int)minQual)),((double )(- 0.1)) * ((double )((int)maxQual)), jacobianLogTable)));
    float temp0 = exp((approximateLog10SumLog10Merlin(((float )(- 0.1)) * ((float )((int)minQual)),((float )(- 0.1)) * ((float )((int)maxQual)), jacobianLogTable)) * LN10);
    //double temp1 = (double)(POW((double)10, log1p(-fmin((double)1.0, temp0)) * (1.0 / log(10))));
...
int printM2MPROB(){
    ...
    for(int k = 0; k < JACOBIAN_LOG_TABLE_SIZE; k++) {
        //jacobianLogTable[k] = (float)(log10(1.0 + POW(10.0, -((float) k) * JACOBIAN_LOG_TABLE_STEP)));
...
FalconPairHMM::FalconPairHMM(){
    useFPGA = false;
    ConvertChar::init();
    /*float jacobianLogTable[JACOBIAN_LOG_TABLE_SIZE];
    ieee754s convert;
    convert.u = LN10_HEX;
    float LN10 = convert.f;
    ...
    for(unsigned char x = 0; x < 128; x++){
        for(unsigned char y = 0; y < 128; y++){
            m2m[index++] = setMatchToMatchProbMerlin(x, y, jacobianLogTable);  
        }
    }*/
...
bool FalconPairHMM::init_FPGA(char* bitstream){
     //get platform info
...
bool FalconPairHMM::init_FPGA_buffer(){
    int err;
    unsigned bankID[4] = {XCL_MEM_DDR_BANK0, XCL_MEM_DDR_BANK1, \
        XCL_MEM_DDR_BANK2, XCL_MEM_DDR_BANK3};

    cl_mem_ext_ptr_t input_ext[3];
    cl_mem_ext_ptr_t output_ext[3];
    input_ext[0].flags = bankID[KERNEL0_BANKID];
    output_ext[0].flags = bankID[KERNEL0_BANKID];
    input_ext[1].flags = bankID[KERNEL1_BANKID];
    output_ext[1].flags = bankID[KERNEL1_BANKID];
    input_ext[2].flags = bankID[KERNEL2_BANKID];
    output_ext[2].flags = bankID[KERNEL2_BANKID];

    for(int i = 0; i < 3; i++){
        input_ext[i].obj = &(host_input[i]->dataPack);
        input_ext[i].param = 0;
        output_ext[i].obj = host_output[i];
        output_ext[i].param = 0;
        _input_buffer[i] = clCreateBuffer(context, CL_MEM_READ_ONLY \
                | CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR, \
                sizeof(InputDataPackOpt), &input_ext[i], &err);
        if(!_input_buffer[i] || err != CL_SUCCESS){
            printf("Warning: failed to allocate CL buffer for input[%d]\n", i);
            return false;
        }
        _output_buffer[i] = clCreateBuffer(context, CL_MEM_WRITE_ONLY \
                | CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR, \
                sizeof(float) * MAX_RSDATA_NUM * MAX_HAPDATA_NUM, &output_ext[i], &err);
        if(!_output_buffer[i] || err != CL_SUCCESS){
            printf("Warning: failed to allocate CL buffer for output[%d]\n", i);\
            return false;
        }
    }
...
bool FalconPairHMM::destroy_FPGA_buffer(){
    for(int i = 0; i < 3; i++){
...
FalconPairHMM::FalconPairHMM(char* bitstream){
    ConvertChar::init();
    _MM_SET_FLUSH_ZERO_MODE(_MM_FLUSH_ZERO_ON);
    ...
    cl_read_time = 0;
    recompute_time = 0;
    /*m2m = (float*)malloc(128 * 128 * sizeof(float));
    float jacobianLogTable[JACOBIAN_LOG_TABLE_SIZE];
    ieee754s convert;
    convert.u = LN10_HEX;
    float LN10 = convert.f;
    ...
    for(unsigned char x = 0; x < 128; x++){
        for(unsigned char y = 0; y < 128; y++){
            m2m[idx++] = setMatchToMatchProbMerlin(x, y, jacobianLogTable);  
        }
    }*/
...
bool FalconPairHMM::computePairhmmFPGA(char* input0, int numRead0, int numHap0, float* output0, char* input1, int numRead1, int numHap1, float* output1, char* input2, int numRead2, int numHap2, float* output2){
    int err;
    struct timespec time1, time2, time_diff;
    ...
    cl_event event_read;
    clock_gettime(CLOCK_REALTIME, &time1);
    /*err = clEnqueueWriteBuffer(commands[0], _input_buffer[0], CL_TRUE, 0, sizeof(InputDataPackOpt), input0, 0, NULL, &event_write0);
    if (err != CL_SUCCESS) {
        printf("Error: Failed to write to device buffer: input0! Code %d\n", err);
        return false;
    }
    err = clEnqueueWriteBuffer(commands[0], _input_buffer[1], CL_TRUE, 0, sizeof(InputDataPackOpt), input1, 0, NULL, &event_write1);
    if (err != CL_SUCCESS) {
        printf("Error: Failed to write to device buffer: input1! Code %d\n", err);
        return false;
    }
    if(DIE_NUM == 3){
        err = clEnqueueWriteBuffer(commands[0], _input_buffer[2], CL_TRUE, 0, sizeof(InputDataPackOpt), input2, 0, NULL, &event_write2);
        if (err != CL_SUCCESS) {
            printf("Error: Failed to write to device buffer: input2! Code %d\n", err);
            return false;
        }
    }
    clWaitForEvents(1, &event_write0);
    clWaitForEvents(1, &event_write1);
    clWaitForEvents(1, &event_write2);*/
    ...
    clSetKernelArg(_pmm_kernel[0], 0, sizeof(cl_mem), &_input_buffer[0]);
    clSetKernelArg(_pmm_kernel[0], 1, sizeof(int), &numRead0);
    clSetKernelArg(_pmm_kernel[0], 2, sizeof(int), &numHap0);
    clSetKernelArg(_pmm_kernel[0], 3, sizeof(cl_mem), &_output_buffer[0]);
    clSetKernelArg(_pmm_kernel[1], 0, sizeof(cl_mem), &_input_buffer[1]);
    clSetKernelArg(_pmm_kernel[1], 1, sizeof(int), &numRead1);
    clSetKernelArg(_pmm_kernel[1], 2, sizeof(int), &numHap1);
    clSetKernelArg(_pmm_kernel[1], 3, sizeof(cl_mem), &_output_buffer[1]);
    clSetKernelArg(_pmm_kernel[2], 0, sizeof(cl_mem), &_input_buffer[2]);
    clSetKernelArg(_pmm_kernel[2], 1, sizeof(int), &numRead2);
    clSetKernelArg(_pmm_kernel[2], 2, sizeof(int), &numHap2);
    clSetKernelArg(_pmm_kernel[2], 3, sizeof(cl_mem), &_output_buffer[2]);   
    ...
    if(curNumCell / cur_kernel_time > peak_kernel_gcups)
        peak_kernel_gcups = curNumCell / cur_kernel_time; 
    kernel_time += cur_kernel_time;
 

    clock_gettime(CLOCK_REALTIME, &time1);
    err = clEnqueueMigrateMemObjects(commands[0], DIE_NUM, _output_buffer, CL_MIGRATE_MEM_OBJECT_HOST, 0, NULL, &event_read);
    if (err != CL_SUCCESS) {
    ...
    /*
    clEnqueueMapBuffer(commands[0], _output_buffer[0], CL_TRUE, CL_MAP_READ, 0, numRead0 * numHap0 * sizeof(float), 1, &event_kernel, NULL, NULL);
    clEnqueueMapBuffer(commands[0], _output_buffer[1], CL_TRUE, CL_MAP_READ, 0, numRead1 * numHap1 * sizeof(float), 1, &event_kernel, NULL, NULL);
    if(DIE_NUM == 3){
        clEnqueueMapBuffer(commands[0], _output_buffer[2], CL_TRUE, CL_MAP_READ, 0, numRead2 * numHap2 * sizeof(float), 1, &event_kernel, NULL, NULL);
    }*/
    //clWaitForEvents(1, &event_read);

   /* err = clEnqueueReadBuffer(commands[0], _output_buffer[0], CL_TRUE, 0, numRead0 * numHap0 * sizeof(float), output0, 0, NULL, &event_read0);
    if (err != CL_SUCCESS) {
        printf("Error: Failed to read from device buffer: output0! Code %d\n", err);
        return false;
    }
    

    err = clEnqueueReadBuffer(commands[0], _output_buffer[1], CL_TRUE, 0, numRead1 * numHap1 * sizeof(float), output1, 0, NULL, &event_read1);
    if (err != CL_SUCCESS) {
        printf("Error: Failed to read from device buffer: output1! Code %d\n", err);
        return false;
    }
    
    if(DIE_NUM == 3){
        err = clEnqueueReadBuffer(commands[0], _output_buffer[2], CL_TRUE, 0, numRead2 * numHap2 * sizeof(float), output2, 0, NULL, &event_read2);
        if (err != CL_SUCCESS) {
            printf("Error: Failed to read from device buffer: output2! Code %d\n", err);
            return false;
        }
        
    }
    clWaitForEvents(1, &event_read0);
    clWaitForEvents(1, &event_read1);
    clWaitForEvents(1, &event_read2);*/
    clWaitForEvents(1, &event_read);
    clock_gettime(CLOCK_REALTIME, &time2);
    time_diff = diff_time(time1, time2);
    ...
    clFlush(commands[0]);
    clFinish(commands[0]);
    //clReleaseEvent(event_write0);
    //clReleaseEvent(event_write1);
    //clReleaseEvent(event_write2);
    clReleaseEvent(event_write);
    clReleaseEvent(event_kernel[0]);
    clReleaseEvent(event_kernel[1]);
    clReleaseEvent(event_kernel[2]);
    //clReleaseEvent(event_read0);
    //clReleaseEvent(event_read1);
    //clReleaseEvent(event_read2);
    return true;
}

double FalconPairHMM::get_kernel_time(){
    return kernel_time;
}

void FalconPairHMM::computePairhmm(pairhmmInput* input, pairhmmOutput* output, bool& usedFPGA){
#ifndef HLS_SIM
    if(useFPGA)
        computePairhmmFalcon(input, output, usedFPGA);
    else
        computePairhmmAVX(input, output, false);
#else
    computePairhmmFalcon(input, output, usedFPGA);
