...
void inline smithWatermanBackTrack(SeqPair *p, int32_t match, int32_t mismatch, int32_t open, int32_t extend, int32_t* E_,int32_t tid)
{
    ...
    for(antiDiag = 1; antiDiag <= (nrow + ncol); antiDiag++)
    {
...
#endif
#endif
        if((overhangStrategy == OVERHANG_STRATEGY_INDEL) || (overhangStrategy == OVERHANG_STRATEGY_LEADING_INDEL))
        {
        ...
        if(ilo == (nrow + 1))
        {
            ...
            if((overhangStrategy == OVERHANG_STRATEGY_SOFTCLIP) || (overhangStrategy == OVERHANG_STRATEGY_IGNORE))
            {
    ...
    if(overhangStrategy == OVERHANG_STRATEGY_INDEL)
...
void inline getCIGAR(SeqPair *p, int16_t *cigarBuf_, int32_t tid)
{
    ...
    else if(overhangStrategy == OVERHANG_STRATEGY_LEADING_INDEL)
    {
        i = max_i;
        j = ncol;
    }
    ...
    if(j < ncol && overhangStrategy == OVERHANG_STRATEGY_SOFTCLIP)
    {
    ...
    if(overhangStrategy == OVERHANG_STRATEGY_IGNORE){
        //if(j != ncol) {
        if(p->update_max_j && j != ncol){ 
            i = nrow;
            segment_length = ncol - max_j;
        }
    }
    int state = 0;
    while((i > 0) && (j > 0))
        ...
        else
        {
            switch(btr & 3)
            {
                case MATCH:
                    ...
                    if(cigarId == 0 && overhangStrategy == OVERHANG_STRATEGY_IGNORE){
                        cigarArray[cigarId * 2 + 1] = segment_length + 1;
                    }
                    else
    ...
    if(overhangStrategy == OVERHANG_STRATEGY_SOFTCLIP)
    {
        if(j > 0)
        {
            cigarArray[cigarId * 2] = SOFTCLIP;
    ...
    else if(overhangStrategy == OVERHANG_STRATEGY_IGNORE)
    {
        if(j > 0)
        {
    ...
    for(i = newId; i >= 0; i--)
    {
        p->cigar->cigarElements[curSize].length = cigarArray[2 * i + 1];
        if(cigarArray[2 * i] == SOFTCLIP)
            p->cigar->cigarElements[curSize].state = STATE_CLIP;
        else
            p->cigar->cigarElements[curSize].state = cigarArray[2 * i]; 
/*
    ...
    p->cigar->CigarElementNum = curSize;
    //p->cigarCount = strlen(p->cigar);
}


int32_t CONCAT(runSWOnePairBT_,SIMD_ENGINE)(int32_t match, int32_t mismatch, int32_t open, int32_t extend,uint8_t *seq1, uint8_t *seq2, int32_t len1, int32_t len2, int8_t overhangStrategy, struct Cigar* cigarRet)
{
