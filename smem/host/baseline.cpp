...
double DRAM_trans_size[BANK_NUM];
int bank_id;
...
void bwt_extend(uint32_t *bwt, const bwtintv_t *ik, bwtintv_t ok[4], int is_back)
{
	...
	ok[3].x[is_back] = ik->x[is_back] + (ik->x[!is_back] <= bwt_primary_baseline && ik->x[!is_back] + ik->x[2] - 1 >= bwt_primary_baseline);
...
int bwt_smem1a(uint32_t *bwt, int len, const uint8_t *q, int x, int min_intv, uint64_t max_intv, bwtintv_v *mem, bwtintv_v *tmpvec[2])
{
    int i, j, c, ret;
    bwtintv_t ik, ok[4];
...
int bwt_smem1a_new(uint32_t *bwt, int len, const uint8_t *q, int x, int min_intv, uint64_t max_intv, bwtintv_v *mem, bwtintv_v *tmpvec[2]){
    int i, j, c, ret;
    bwtintv_t ik, ok[4];
    bwtintv_v a[2], *curr;
    int start = x;
    ...
    while (i < curr->n){
        ...
        int max_len;
        if (i < curr->n)
            max_len = (temp.info>>32) + (int)curr->a[i].info;
	...
	if (max_mem_intv > 0) {
		...
		while (x < len) {
			if (seq[x] < 4) {
				if (1) {
					bwtintv_t m;
					x = bwt_seed_strategy1(bwt, len, seq, x, MIN_SEED_LEN, max_mem_intv, &m);
...
int smem_baseline(const uint32_t* bwt, const uint64_t* bwt_para, uint8_t* seq, uint8_t* seq_len, int batch_size, bwtintv_t* mem_output, int* mem_num, double mem_request_size[BANK_NUM]){
    ...
    bank_id = 0;
    int bank_seg_size = batch_size / BANK_NUM;
    int count = 0;
    for (int i = 0; i < batch_size; i++) {
        smem_aux_t mem_aux;
        kv_init(mem_aux.mem);
        kv_init(mem_aux.mem1);
        mem_collect_intv_new((uint32_t*)bwt, seq_len[i], seq + i * SEQ_LENGTH, &mem_aux);
        mem_num[i] = mem_aux.mem.n;
        //printf("batch %d, number of intv is %d, seq_len is %d\n", i, mem_num[i], seq_len[i]);
        int mem_n_amend = mem_aux.mem.n;
        if(mem_n_amend > MAX_INTV_ALLOC) {
            mem_n_amend = MAX_INTV_ALLOC;
        }
        for (int j = 0; j < mem_n_amend; j++) { 
            mem_output[i * MAX_INTV_ALLOC + j] = mem_aux.mem.a[j];
        }
...
/*
int baseline_run(const uint32_t *bwt, char* fname_input, char* fname_golden, uint64_t& time_total) {
    bwt_primary_baseline = BWT_PRIMARY;
    L2_baseline[0] = L2[0];
    L2_baseline[1] = L2[1];
    L2_baseline[2] = L2[2];
    L2_baseline[3] = L2[3];
    ...
    seq = (uint8_t*)malloc(len * sizeof(uint8_t) * BATCH_SIZE);
    fread(seq, sizeof(uint8_t), len, finput);
    fclose(finput);
    for (int i = 1; i < BATCH_SIZE; i++) {
        memcpy(&seq[i*len], seq, len*sizeof(uint8_t));
    }

    int mem_num[BATCH_SIZE];
    bwtintv_t mem_output[MAX_INTV_ALLOC * BATCH_SIZE];
    smem_aux_t aux[BATCH_SIZE];
    for(int i = 0; i < BATCH_SIZE; i++){
        kv_init(aux[i].mem);
        mem_collect_intv_new((uint32_t*)bwt, len, &seq[i * len], &aux[i]);
    }
    int wrong = 0;
    for (int k = 0; k < BATCH_SIZE; k++) {
        ...
        if (n != aux[k].mem.n) {
            fclose(fgolden);
            printf("there are %d intv in the result but %d in the golden.\n", aux[k].mem.n, n);
            return 1;
        }
        else {
            for (int i=0; i<n; i++) {
                ...
                if (golden_x0 != aux[k].mem.a[i].x[0])
                    printf("x0 is different for intv %d\n", i);
                if (golden_x1 != aux[k].mem.a[i].x[1])
                    printf("x1 is different for intv %d\n", i);
                if (golden_x2 != aux[k].mem.a[i].x[2])
                    printf("x2 is different for intv %d\n", i);
                if(golden_info != aux[k].mem.a[i].info) {
